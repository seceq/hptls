...
use hpcrypt_hpke::{HpkeP256, HpkeX25519};
use hptls_crypto::{
...
/// Internal enum for different HPKE KEM implementations
enum HpkeImplementation {
    P256(HpkeP256),
    X25519(HpkeX25519),
}
...
impl HpcryptHpke {
    ...
    pub fn new(cipher_suite: HpkeCipherSuite) -> Result<Self> {
        ...
        // Create the appropriate HPKE instance based on KEM and AEAD
        let hpke = match cipher_suite.kem {
            HpkeKem::DhkemP256HkdfSha256 => {
                let p256 = match cipher_suite.aead {
                ...
                HpkeImplementation::P256(p256)
            }
            HpkeKem::DhkemX25519HkdfSha256 => {
                let x25519 = match cipher_suite.aead {
                    HpkeAead::Aes128Gcm => HpkeX25519::new(),
                    HpkeAead::Aes256Gcm => HpkeX25519::with_aes256(),
                    HpkeAead::ChaCha20Poly1305 => HpkeX25519::with_chacha(),
                    _ => {
                        return Err(Error::UnsupportedAlgorithm(format!(
                            "AEAD {:?} not supported",
                            cipher_suite.aead
                ...
                HpkeImplementation::X25519(x25519)
            }
            _ => {
                return Err(Error::UnsupportedAlgorithm(format!(
                    "KEM {:?} not supported",
...
impl Hpke for HpcryptHpke {
    ...
    fn generate_keypair(&self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        match &self.hpke {
            HpkeImplementation::P256(_) => HpkeP256::generate_keypair(&mut rng),
            HpkeImplementation::X25519(_) => HpkeX25519::generate_keypair(&mut rng),
        }
        .map_err(|e| Error::CryptoError(format!("HPKE keypair generation failed: {:?}", e)))
    }
    ...
    fn setup_base_sender(
        ...
        let mut rng = rand::thread_rng();
        let (enc, ctx) = match &self.hpke {
            HpkeImplementation::P256(hpke) => hpke.setup_base_sender(pk_r, info, &mut rng),
            HpkeImplementation::X25519(hpke) => hpke.setup_base_sender(pk_r, info, &mut rng),
        }
        .map_err(|e| Error::CryptoError(format!("HPKE setup_base_sender failed: {:?}", e)))?;
    ...
    fn setup_base_recipient(
        ...
        let ctx = match &self.hpke {
            HpkeImplementation::P256(hpke) => hpke.setup_base_recipient(enc, sk_r, info),
            HpkeImplementation::X25519(hpke) => hpke.setup_base_recipient(enc, sk_r, info),
        }
        .map_err(|e| Error::CryptoError(format!("HPKE setup_base_recipient failed: {:?}", e)))?;
    ...
    fn seal_base(
        ...
        let mut rng = rand::thread_rng();
        match &self.hpke {
            HpkeImplementation::P256(hpke) => {
                hpke.seal_base(pk_r, info, aad, plaintext, &mut rng)
            }
            HpkeImplementation::X25519(hpke) => {
                hpke.seal_base(pk_r, info, aad, plaintext, &mut rng)
            }
        }
    ...
    fn open_base(
        ...
        match &self.hpke {
            HpkeImplementation::P256(hpke) => hpke.open_base(enc_and_ciphertext, sk_r, info, aad),
            HpkeImplementation::X25519(hpke) => {
                hpke.open_base(enc_and_ciphertext, sk_r, info, aad)
            }
        }
